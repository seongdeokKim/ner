...
class Arguments():
    ...
    def __init__(self):
        ...
        self.train_fn = './data/_reddit_CoNLL2003_final.txt'
        #self.train_fn = './data/reddit_CoNLL2003_final.txt'
...
def get_loaders(config, tokenizer):
    ...
    tag_to_index = {t: i for i, t in enumerate(tag_list)}
    index_to_tag = {i: t for i, t in enumerate(tag_list)}
...
def get_model(config, index_to_tag):

    if config.model_type == 'bert':
        model = BertForTokenClassification.from_pretrained(config.pretrained_model_name,
                                                           num_labels=len(index_to_tag))

    elif config.model_type == 'bert_bilstm':
        model = BERT_BiLSTM(config,
                            num_labels=len(index_to_tag))
        ...
        raise ValueError("Your model type is wrong. please put exact model type")
...
def get_trainer(config):
    ...
    if 'crf' not in config.model_type:
...
def main(config):
    ...
    # Get language model
    model = get_model(config, index_to_tag)
